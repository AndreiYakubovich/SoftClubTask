...
namespace ConsoleCore
{
    public class CompareAlgorithms
    {
        ...
        public static void GetIntersectionByFields(IEnumerable<Element> firstCollection, IEnumerable<Element> secondCollection)
        {
            ...
            timer.Start();
            IEnumerable<Element> resultList = GetIntersection(firstCollection, secondCollection, new ElementEqualityComparer())
                .ToList();
            timer.Stop();
            Console.WriteLine(
                $"time of the field comparison algorithm = {timer.ElapsedMilliseconds} milliseconds, coincidence percentage = {GetPercentOfDifference(resultList, firstCollection)}%{Environment.NewLine}");
        }
            ...
            IEnumerable<Element> secondCollection, methodedelegade methode)
        {
            ...
            timer.Stop();
            Console.WriteLine(
                $"Time of the algorithm = {timer.ElapsedMilliseconds} milliseconds,coincidence percentage = {GetPercentOfDifference(resultList, firstCollection)}%{Environment.NewLine}");
        }
            ...
            IEnumerable<Element> secondCollection)
        {
            ...
            foreach (var firstventorgroup in firstDictionary)
            {
                if (secondDictionary.TryGetValue(firstventorgroup.Key, out List<Element> secondvendorgroup))
                {
                    foreach (var element in firstventorgroup.Value)
                    {
